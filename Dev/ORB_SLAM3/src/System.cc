...
#include <vector>
#include <Eigen/Dense>
std::vector<Eigen::Vector3f> ROS_POINTCLOUD;
cv::Mat ROS_IMAGE;
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
ROS2_OUTPUT System::TrackMonocular_ROS(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ROS2_OUTPUT DATA;
    {
        unique_lock<mutex> lock(mMutexReset);
    ...
    Sophus::SE3f ROS_Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename);
    ...
    DATA.pos        = ROS_Tcw;
    DATA.image      = ROS_IMAGE;
    DATA.pointcloud = ROS_POINTCLOUD;
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
ROS2_OUTPUT System::TrackStereo_ROS(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ROS2_OUTPUT DATA;
    ...
    // std::cout << "start GrabImageStereo" << std::endl;
    Sophus::SE3f ROS_Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename);
