...
std::vector<Eigen::Vector3f> MapDrawer::DrawMapPoints()
{
    // Create a vector to store the 3D points
    std::vector<Eigen::Vector3f> point_cloud;

    // Get the current active map from the atlas
    Map* pActiveMap = mpAtlas->GetCurrentMap();
    if(!pActiveMap) // If no active map, return the empty point cloud
        return point_cloud;

    // Retrieve all map points and reference map points from the active map
    ...
    // Convert reference map points vector to a set for quick lookup
    std::set<MapPoint*> spRefMPs(vpRefMPs.begin(), vpRefMPs.end());

    if(vpMPs.empty()) // If no map points, return the empty point cloud
        return point_cloud;

    // Render non-reference map points in black
    ...
    {
        // Skip bad points and reference points
    ...
    // Render reference map points in red and add them to the point cloud
    glPointSize(mPointSize);
    glBegin(GL_POINTS);
    ...
    {
        if((*sit)->isBad()) // Skip bad points
            continue;

        // Get the 3D position of the reference map point
