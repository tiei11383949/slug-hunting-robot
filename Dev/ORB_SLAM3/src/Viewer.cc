...
#include <vector>
#include <Eigen/Dense>
extern std::vector<Eigen::Vector3f> ROS_POINTCLOUD;
extern cv::Mat ROS_IMAGE;
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
...
void Viewer::Run()
{
    ...
    while(1)
    {
        ...
        if(menuShowPoints)
            ...
            ROS_POINTCLOUD = mpMapDrawer->DrawMapPoints();
...
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if(both){
            ...
            cv::hconcat(im,imRight,ROS_IMAGE);
        }
        else{
            ROS_IMAGE = im;
        }
        ...
        {
            int width = ROS_IMAGE.cols * mImageViewerScale;
            int height = ROS_IMAGE.rows * mImageViewerScale;
            cv::resize(ROS_IMAGE, ROS_IMAGE, cv::Size(width, height));
        }

        cv::imshow("ORB-SLAM3: Current Frame",ROS_IMAGE);
        cv::waitKey(mT);
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
// --------------------------------------------------------------------------------------------------------------------------------------------------------------
